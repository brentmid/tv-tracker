...
- [x] **M9 Importer — code + offline verification done; real import pending Brent's go**
  ...
  - [ ] Wipe dev-seeded `baselines/tvtracker.db` (ASK BRENT — contains dev seed + Severance + Inception; all re-importable) → `commit` → resolve stragglers at `/import` → validate counts
